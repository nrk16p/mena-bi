...
import { buildMonthFuelQty, monthKeyOf } from "@/lib/trip-count/calculate"
import { DELIVER_DB, FUEL_QTY_DATA_COLLECTION, fetchDriverCostRows } from "@/lib/trip-count/source"
...
export async function POST(req: NextRequest) {
  ...
  await col.createIndex({ monthKey: 1, fuelType: 1 })
  ...
  const results = await runEtlMonths(db, {
    ...
    processMonth: async (year, month, startedAt) => {
      ...
      const { monthKey, rowsInMonth, docs, totalOil, totalNgv, byFuelType, excluded } =
        buildMonthFuelQty(rows, year, month, ruleDoc.rules)
